...
const InvestmentPage = () => {
  ...
  useEffect(() => {
    const carregarInvestimentos = async () => {
      try {
        const response = await investmentService.listar();
        setPurchases(response.data.investments || []);
      } catch (err) {
        console.error(
  ...
  const portfolioChartData = useMemo(() => {
    ...
    return sortedPurchases.map((p) => {
      ...
      cumulativeValue += val;
      return { date: p.date, totalValue: cumulativeValue };
    });
  }, [purchases]);
  ...
  const handleAddPurchase = async (e) => {
    ...
    try {
      // Ajuste: enviando os dados exatamente como o backend espera
      await investmentService.adicionar({
        name: ticker,
        type: "Ação",
      ...
      console.error("Erro completo ao salvar:", err.response?.data);
      alert(
        "Erro ao salvar: " +
          (err.response?.data?.message || "Verifique o console do navegador"),
      );
    }
  };
              ...
              <div className="portfolio__assets-list">
                ...
                <div className="assets-list__container">
                  {purchases.map((asset) => (
                    <div key={asset.id} className="asset-item">
                      <div className="asset-item__info">
                        <span className="asset-item__ticker">{asset.name}</span>
                      </div>
                      <div className="asset-item__details">
                        <strong>R$ {parseFloat(asset.value).toFixed(2)}</strong>
                      </div>
                      <button
                        className="btn-delete"
                        onClick={() => handleDeleteAsset(asset.id)}
